...
public class OrderedList_inArraySlots
    ...
    public int whileStyle (Integer findMe) {
      for (int index = 0; index < list_iAS.toArray().length; index++) {
        if (list_iAS.get(index) == findMe)
          return index;
        }
      return -1;
    }

     public int recursive( Integer findMe, int low, int high) {
       while (low <= high){
         int pageToCheck = (high + low)/2
         if (list_iAS.get(pageToCheck) == findMe) return pageToCheck;
         else if (list_iAS.get(pageToCheck) > findMe){
           high = pageToCheck - 1;
           return recursive(findMe, low, high);
         }
         else {
           low = pageToCheck + 1;
           return recursive(findMe, low, high);
